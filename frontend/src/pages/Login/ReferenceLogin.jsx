...
import { Alert, Box, Button, CircularProgress, Divider, InputAdornment, Paper, Stack, TextField, ToggleButton, ToggleButtonGroup, Typography, useTheme } from "@mui/material";
...
export default function ReferenceLogin() {
  ...
  return <Box className="clorox-login"><Box sx={{ position: "fixed", zIndex: 2, top: { xs: 14, sm: 22 }, left: { xs: 14, sm: 22 } }}><ThemeModeToggle /></Box><Paper component="form" onSubmit={submit} elevation={0} className="clorox-login-card" sx={{ width: "min(100%, 540px)", px: { xs: 3, sm: 7 }, py: { xs: 4, sm: 5.5 }, borderRadius: { xs: 4, sm: 5 }, bgcolor: isDark ? "#18222f" : "rgba(255,255,255,.97)", boxShadow: "0 30px 70px rgba(0,51,132,.28)", border: "1px solid", borderColor: isDark ? "#36485c" : "rgba(255,255,255,.75)" }}><Stack alignItems="center" spacing={1.1}><Box component="img" src="/clorox.png" alt="Clorox" sx={{ width: 165, maxHeight: 100, objectFit: "contain" }} /><Typography sx={{ color: isDark ? "#e6efff" : "#0a2c69", fontWeight: 900, fontSize: { xs: 34, sm: 42 }, lineHeight: 1 }}>Sales App</Typography><Typography sx={{ color: isDark ? "#b6c5d8" : "#617095", fontSize: { xs: 17, sm: 20 }, fontWeight: 700 }}>بوابة تسجيل الدخول</Typography></Stack><Stack spacing={2.1} sx={{ mt: 3.2 }}>{error && <Alert severity="error" onClose={() => setError("")}>{error}</Alert>}<ToggleButtonGroup value={role} exclusive onChange={selectRole} fullWidth size="small" aria-label="نوع الحساب" sx={{ "& .MuiToggleButton-root": { flex: 1, gap: .45, py: 1, borderColor: isDark ? "#384a5e" : "#dce5f2", fontWeight: 800, fontSize: { xs: 12, sm: 14 } }, "& .Mui-selected": { color: "#78adff !important", bgcolor: isDark ? "#20324a !important" : "#eef5ff !important" } }}>{roles.map((item) => <ToggleButton key={item.value} value={item.value}>{item.icon}{item.label}</ToggleButton>)}</ToggleButtonGroup><Stack direction="row" spacing={1.5} alignItems="center" sx={{ width: "100%" }}><Divider flexItem sx={{ my: "auto" }} /><Box sx={{ width: 62, height: 62, borderRadius: "50%", display: "grid", placeItems: "center", color: "#4e94ff", bgcolor: isDark ? "#21334b" : "#e7f0ff" }}>{activeRole.icon}</Box><Divider flexItem sx={{ my: "auto" }} /></Stack><TextField fullWidth label={activeRole.codeLabel} placeholder="أدخل الكود" value={accessCode} onChange={(event) => setAccessCode(event.target.value)} autoComplete="username" inputMode="text" inputProps={{ autoCapitalize: "characters", spellCheck: false }} InputProps={{ startAdornment: <InputAdornment position="start"><BadgeOutlinedIcon color="primary" /></InputAdornment> }} /><TextField fullWidth label="الرمز السري" placeholder="أدخل الرمز السري" value={secretCode} onChange={(event) => setSecretCode(event.target.value)} type="password" autoComplete="current-password" InputProps={{ startAdornment: <InputAdornment position="start"><LockOutlinedIcon color="primary" /></InputAdornment> }} /><Button size="large" type="submit" variant="contained" disabled={loading} endIcon={loading ? <CircularProgress size={20} color="inherit" /> : <LoginRoundedIcon />} sx={{ mt: .4, py: 1.45, fontSize: 18, fontWeight: 800, borderRadius: 2.5, bgcolor: "#075be1", boxShadow: "0 11px 18px rgba(0,62,167,.25)", "&:hover": { bgcolor: "#064db8" } }}>{loading ? "جارٍ التحقق..." : "تسجيل الدخول"}</Button></Stack></Paper></Box>;
}
