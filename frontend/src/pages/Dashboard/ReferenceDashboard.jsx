...
import { Alert, Avatar, Box, Button, Card, CardContent, CircularProgress, Dialog, DialogContent, DialogTitle, Divider, IconButton, Stack, Typography } from "@mui/material";
...
function formatVacationDate(value, options) {
  const date = new Date(`${String(value || "").slice(0, 10)}T12:00:00`);
  if (Number.isNaN(date.getTime())) return "غير محدد";
  return new Intl.DateTimeFormat("ar-EG", { timeZone: "Africa/Cairo", ...options }).format(date);
}

function VacationDays({ days }) {
  return <Box sx={{ mt: 2.25 }}><Typography fontWeight={900} sx={{ mb: 1.1 }}>أيام الإجازة المسجلة</Typography>{days?.length ? <Stack divider={<Divider flexItem />} sx={{ borderTop: "1px solid #e5ebf3", borderBottom: "1px solid #e5ebf3" }}>{days.map((day) => <Box key={day.id} sx={{ py: 1.35 }}><Box sx={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", textAlign: "center", gap: 1 }}><Box><Typography variant="caption" color="text.secondary">اليوم</Typography><Typography fontWeight={800}>{formatVacationDate(day.date, { weekday: "long" })}</Typography></Box><Box sx={{ borderInline: "1px solid #e5ebf3" }}><Typography variant="caption" color="text.secondary">التاريخ</Typography><Typography fontWeight={800}>{formatVacationDate(day.date, { year: "numeric", month: "2-digit", day: "2-digit" })}</Typography></Box><Box><Typography variant="caption" color="text.secondary">الشهر</Typography><Typography fontWeight={800}>{formatVacationDate(day.date, { month: "long", year: "numeric" })}</Typography></Box></Box>{day.vacationType && <Typography variant="caption" color="text.secondary" textAlign="center" display="block" sx={{ mt: 1 }}>{day.vacationType}</Typography>}</Box>)}</Stack> : <Typography color="text.secondary" variant="body2">لا توجد أيام إجازة سنوية مسجلة حتى الآن.</Typography>}</Box>;
}
...
export default function ReferenceDashboard() {
  ...
  const reportsSummary = dashboard?.reportsSummary || summary;
  ...
  const cards = [
    ...
    { key: "reports", title: "تقارير الشهر", color: "#8b5cf6", tint: "#faf8ff", icon: <DescriptionOutlinedIcon />, items: [{ label: "تقارير مبيعات", value: number(reportsByType.Sales), icon: <DescriptionOutlinedIcon fontSize="inherit" /> }, { label: "تقارير فاوتشر", value: number(reportsByType.Vouchers), icon: <ReceiptLongOutlinedIcon fontSize="inherit" /> }, { label: "تقارير إجازة", value: number(reportsByType.Vacation), icon: <BeachAccessOutlinedIcon fontSize="inherit" /> }], footer: `إجمالي التقارير: ${number(reportsSummary.count)}  |  قيمة المبيعات: ${money(reportsSummary.salesValue)}` },
    { key: "vacation", title: "الإجازات السنوية", color: "#e59a27", tint: "#fffbf4", icon: <BeachAccessOutlinedIcon />, items: [{ label: "الإجمالي", value: number(vacation.total), icon: <EventAvailableRoundedIcon fontSize="inherit" /> }, { label: "المستنفذ", value: number(vacation.consumed), color: "#c26425", icon: <HourglassBottomRoundedIcon fontSize="inherit" /> }, { label: "المتبقي", value: number(vacation.remaining), color: "#16825a", icon: <CheckCircleOutlineRoundedIcon fontSize="inherit" /> }], vacationDays: vacation.days || [] },
    { key: "salesValue", title: "قيمة المبيعات", color: "#0f766e", tint: "#f0fdfa", icon: <PaidRoundedIcon />, items: [{ label: "اليوم", value: money(summary.salesValue), icon: <PaidRoundedIcon fontSize="inherit" /> }, { label: "هذا الشهر", value: money(reportsSummary.salesValue), icon: <DescriptionOutlinedIcon fontSize="inherit" /> }, { label: "التراكمي", value: money(cumulative.salesValue), icon: <AutoGraphRoundedIcon fontSize="inherit" /> }], details: [{ label: "قيمة مبيعات اليوم", value: money(summary.salesValue) }, { label: "قيمة مبيعات الشهر", value: money(reportsSummary.salesValue) }, { label: "قيمة المبيعات التراكمية", value: money(cumulative.salesValue) }], footer: "تُحسب من القطع المسجلة وسعر المنتج في شهر التقرير" },
  ];
  const activeCard = [cumulativeCard, ...cards].find((card) => card.key === selectedCard);

  return <AppShell hideHeader>
    <Box className="delegate-screen-title">
      <Box><Typography className="delegate-screen-title__eyebrow">لوحة المتابعة اليومية</Typography><Typography variant="h5" fontWeight={900}>الرئيسية</Typography></Box>
      <Button size="small" variant="outlined" onClick={() => navigate("/product-guide")} endIcon={<AutoStoriesRoundedIcon />} sx={{ borderColor: "#cfe0f8", color: "#175fc7", fontWeight: 900 }}>دليل المنتجات</Button>
    </Box>
    <Card elevation={0} className="clorox-hero" sx={{ minHeight: 184, overflow: "hidden", borderRadius: { xs: 4, sm: 5 }, color: "#fff", boxShadow: "0 14px 30px rgba(22,86,186,.24)" }}>
      <CardContent sx={{ minHeight: 184, p: { xs: 2.25, sm: 3 }, display: "flex", alignItems: "center" }}>
        <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={1.4} sx={{ width: "100%" }}>
          <Stack direction="row" alignItems="center" spacing={1.35} sx={{ minWidth: 0 }}>
            <Avatar src={user?.avatarUrl || undefined} sx={{ width: { xs: 60, sm: 68 }, height: { xs: 60, sm: 68 }, border: "3px solid rgba(255,255,255,.55)", bgcolor: "rgba(255,255,255,.18)", fontSize: 25, fontWeight: 900 }}>{name.charAt(0)}</Avatar>
            <Box sx={{ minWidth: 0 }}><Stack direction="row" spacing={.65} alignItems="center"><WavingHandRoundedIcon sx={{ fontSize: 23, color: "#ffd269", transform: "rotate(-10deg)" }} /><Typography sx={{ opacity: .88, fontWeight: 700 }}>{greeting}</Typography></Stack><Typography noWrap sx={{ mt: .3, fontSize: { xs: 23, sm: 29 }, lineHeight: 1.25, fontWeight: 900 }}>{name}</Typography><Stack direction="row" spacing={.55} alignItems="center" sx={{ mt: .7, opacity: .86 }}><CalendarMonthRoundedIcon sx={{ fontSize: 16 }} /><Typography variant="caption" fontWeight={700}>{date}</Typography></Stack></Box>
          </Stack>
          <Box sx={{ flexShrink: 0, minWidth: { xs: 72, sm: 102 }, px: 1.1, py: 1, border: "1px solid rgba(255,255,255,.24)", borderRadius: 3, bgcolor: "rgba(255,255,255,.12)", textAlign: "center" }}><Typography variant="caption" sx={{ opacity: .82, display: "block" }}>إنجاز العملاء</Typography><Typography fontWeight={900}>{percentage(customers)}</Typography></Box>
        </Stack>
      </CardContent>
    </Card>
