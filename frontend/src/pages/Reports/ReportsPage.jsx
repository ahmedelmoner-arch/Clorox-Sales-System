...
function SalesValueCard({ value, month }) {
  return <Card elevation={0} sx={{ mb: 2, overflow: "hidden", borderRadius: 4, color: "#fff", background: "linear-gradient(115deg, #0f766e 0%, #14b8a6 100%)", boxShadow: "0 10px 22px rgba(13,148,136,.18)" }}><CardContent sx={{ p: { xs: 1.8, sm: 2.2 } }}><Stack direction="row" alignItems="center" justifyContent="space-between" spacing={1.5}><Box sx={{ minWidth: 0, flex: 1 }}><Typography variant="body2" sx={{ opacity: .84, fontWeight: 700 }}>قيمة المبيعات المحققة</Typography><Typography sx={{ mt: .3, fontSize: { xs: "clamp(1.35rem, 7vw, 1.7rem)", sm: 32 }, lineHeight: 1.15, fontWeight: 900, fontVariantNumeric: "tabular-nums", overflowWrap: "anywhere" }}>{money(value)}</Typography><Typography variant="caption" sx={{ display: "block", mt: .6, opacity: .84 }}>محسوبة بأسعار UnitPrice لشهر وتاريخ كل تقرير في {formatMonth(month)}</Typography></Box><Box sx={{ width: 52, height: 52, flex: "0 0 auto", display: "grid", placeItems: "center", borderRadius: 3, bgcolor: "rgba(255,255,255,.16)" }}><PaidRoundedIcon sx={{ fontSize: 29 }} /></Box></Stack></CardContent></Card>;
}
...
export default function ReportsPage({ forcedType }) {
  ...
  return <AppShell hideHeader>
    <Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" alignItems={{ sm: "center" }} spacing={1.5} sx={{ mb: 2.25 }}><Box><Typography className="delegate-screen-title__eyebrow">كل الزيارات المسجلة</Typography><Typography variant="h5" fontWeight={900}>{forcedType === "Vacation" ? "الإجازات" : "التقارير السابقة"}</Typography><Typography color="text.secondary" variant="body2">{forcedType === "Vacation" ? "سجل الإجازات المعتمد للمندوبة." : "اختاري أي زيارة لعرض تفاصيلها أو تعديلها."}</Typography></Box><TextField type="month" size="small" label="الشهر" value={month} onChange={(event) => setMonth(event.target.value)} InputLabelProps={{ shrink: true }} sx={{ width: { xs: "100%", sm: 172 }, "& .MuiOutlinedInput-root": { borderRadius: 3 } }} /></Stack>
